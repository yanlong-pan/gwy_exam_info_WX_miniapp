<template>
	...
	<block v-if="node.node == 'element'">
		...
		<view v-else-if="node.tag == 'a'" @click="wxParseATap(node.attr,$event)" :class="node.classStr"
			...
			style="display: inline; border-bottom: 1px solid #555555;">
		...
		<!--strong标签-->
		<view v-else-if="node.tag == 'strong'" :class="node.classStr" :style="node.styleStr"
		...
		<!--em标签-->
		<view v-else-if="node.tag == 'em'" :class="node.classStr" :style="node.styleStr"
